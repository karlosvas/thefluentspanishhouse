...
import { updateProfile, getAuth, signInWithPopup, signInWithEmailAndPassword, signOut, createUserWithEmailAndPassword, GoogleAuthProvider, fetchSignInMethodsForEmail
} from 'firebase/auth';
...
export function signInWithGoogle(toggleModal: (type: string) => void) {
   signInWithPopup(auth, providerGoogle)
     .then((result) => {
       ...
       toast.success(<span>Welcome back <b>{user}</b>!</span>);
       toggleModal('');
     })
     .catch((error) => {
     ...
     const userCredential = await signInWithPopup(auth, providerGoogle);
      ...
      if(email){
         ...
         if (signInMethods && signInMethods.length > 0)
            ...
            toast.success(<span>Welcome <b>{userCredential.user.displayName}</b>!</span>);
      } else
         toast.error("The user does not have an email");
   } catch (error) {
...
export const localRegister = async (email: string, password: string, username: string) => {
   if(isLogged()) {
      toast.error('You are already logged in')
      return
   }
...
// Deslogearse
export function signOutUser(buttonRef: React.RefObject<HTMLButtonElement> | null) {
   signOut(auth)
      .then(() => {
