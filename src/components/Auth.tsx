...
function Auth() {
    ...
    const loginOrLogout = () => {
        isLogged() ? signOutUser(logRef) : toggleModal('login');
    };

    const googleAuth = () =>{
        console.log(typeLoginRegisterRef.current?.textContent);
        (typeLoginRegisterRef.current?.textContent === 'Register') ? registerWithGoogle() : signInWithGoogle(toggleModal)
    }
    
    return (
        <>
            <button id="singIn" onClick={loginOrLogout} ref={logRef}>
                {isLogged() ? 'Logout' : 'Sing in'}
            </button>
            <button onClick={() => toggleModal('register')}  id="register">
                Register
            </button>
            {showModal &&
            <>
                ...
                <div className="modalAuth">
                    ...
                    <span className="closeAuth" onClick={() => toggleModal('')}>&times;</span>
                        <h2 ref={typeLoginRegisterRef}>{formType === 'login' ? 'Login' : 'Register'}</h2>
                        ...
                        <div className="googleAuth">
                            Continuar con
                            <svg onClick={googleAuth} id='svgGoogle' xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20" height="20" viewBox="0 0 48 48">
