...
//####################### GET #######################
test('/getall/member obtener todos los miembros de una lista', async ({
  page,
}) => {
...
test('/groupscategory obtener todos los grupos de categorias', async ({
  page,
}) => {
...
test.describe.serial('Mailchimp tests', () => {
  ...
  test('/get/interests obtener los intereses de un grupo de categorias', async ({
    page,
  }) => {
  ...
  test('/getone/member/:email obtener un miembro', async ({ page }) => {
    const responseMember = await page.goto(
      `/mailchimp/getone/member/${testEmail}`
    );
    expect(responseMember?.status()).toBe(200);
  ...
  //####################### POST #######################
  test('/add/batchcontact añadir varios miembros a la lista (opcional)', async ({
    page,
  }) => {
    ...
    const responseNewMembers = await page.request.post(
      `/mailchimp/add/batchcontact`,
      {
        data: members,
      }
    );
  ...
  test('/add/interests añadir intereses', async ({ page }) => {
    ...
    const responseNewInterests = await page.request.post(
      `mailchimp/add/interests`,
      {
        data: { name },
      }
    );
    ...
    if (
      data.detail &&
      data.detail ==
        `Cannot add "${name}" because it already exists on the list.`
    ) {
      // Si el interés ya existe, no se puede añadir asique lo buscamos
      const responseAllInterests = await page.request.get(
        `/mailchimp/get/interests`
      );

      const dataInterest = await responseAllInterests?.json();
      const element = dataInterest.interests.find(
  ...
  //####################### PUT #######################
  test('/updatecontact/status/:email editar estado de un usuario por email', async ({
    page,
  }) => {
    const status: Status = 'transactional';
    const responseStatus = await page.request.put(
      `/mailchimp/updatecontact/status/${testEmail}`,
      {
        data: { status },
      }
    );
  ...
  test('/updatecontact/tag/:email editar tag de un usuario por email', async ({
    page,
  }) => {
    const tag: OptionsChampTag = 'GROUP_CLASS';
    const responseTag = await page.request.put(
      `/mailchimp/updatecontact/tag/${testEmail}`,
      {
        data: { tag },
      }
    );
  ...
  test('/del/interests/:id eliminar intereses de una categoria', async ({
    page,
  }) => {
    // Verificamos si existe el interes de testing y obtenemos su id
    if (idInterest == '') console.warn('Dont exist the interest testing');
    else {
      const responseInterests = await page.request.delete(
        `/mailchimp/del/interests/${idInterest}`
      );
  ...
  test('/del/tag/:email eliminar tag de un miembro', async ({ page }) => {
    const tag: OptionsChampTag = 'GROUP_CLASS';
    const responseDelTag = await page.request.delete(
      `/mailchimp/del/tag/${testEmail}`,
      {
        data: { tag },
      }
    );
  ...
  test('/del/user/:email eliminar usuarios', async ({ page }) => {
    const responseUser = await page.request.delete(
      `/mailchimp/del/member/${testEmail}`
    );
    expect(responseUser?.status()).toBe(204);
    const responseUser1 = await page.request.delete(
      `/mailchimp/del/member/testing1@gmail.com`
    );
    expect(responseUser1?.status()).toBe(204);
    const responseUser2 = await page.request.delete(
      `/mailchimp/del/member/testing2@gmail.com`
    );
