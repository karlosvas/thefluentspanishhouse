...
router.get("/getall/member", log, verifyIdToken, async (_req: Request, res: Response) => {
  mailchimp.lists
...
router.delete("/del/member/:email", log, verifyIdToken, async (req: Request, res: Response) => {
...
router.delete("/del/interests/:id", log, verifyIdToken, async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    ...
    if (isErrorAxios(error)) res.status(error.status).json(error.message);
